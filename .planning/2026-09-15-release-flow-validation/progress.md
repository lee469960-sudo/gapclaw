...
- 2026-09-16: User selected option 1, rerun the existing `v1.0.24` failed job. Retried the GitHub rerun API again and received the same 403 `Resource not accessible by personal access token`; production remains ready, but rerun must be started from GitHub UI or a token with Actions write permission.
- 2026-09-16: User manually reran `v1.0.24` run `34994891180` attempt 2. The workflow rebuilt and pushed API/Web images, generated/uploaded the immutable manifest, then failed again at signed Hook delivery with HTTP 403. This confirms rerun happened, but GitHub Actions `GAP_RELEASE_HOOK_SECRET` is still not aligned with production's new Hook secret.
- 2026-09-16: User confirmed the intended long Hook secret value. Production was rechecked and remains healthy (`/health` 200) with unsigned Hook returning 403 `release_hook_signature_invalid`. Browser automation for setting the GitHub Actions secret failed during CUA initialization, so the repository secret still needs to be set manually or with an authorized GitHub token.
- 2026-09-16: User requested continued monitoring. GitHub run `34994891180` reached attempt 4 and still failed at Hook delivery with HTTP 403. A production-side signed probe using the API container's own `RELEASE_HOOK_SECRET` returned 403 `release_hook_manifest_invalid` (not signature invalid), proving production signing configuration is correct and the remaining mismatch is GitHub Actions secret value.

...
- 2026-09-16: User supplied a new 32+ character Hook secret. Updated production `/opt/gap/.env`, rebuilt `gap-production-api-1` against the current v1.0.21 immutable images, and verified `/health` returns 200 while unsigned Hook POST returns 403 `release_hook_signature_invalid`. Production is healthy and ready for a matching GitHub Actions secret.
- 2026-09-16: Retried GitHub failed-job rerun for `v1.0.24` run `34994891180`; current PAT still returns 403 `Resource not accessible by personal access token`. No new GitHub run was started.

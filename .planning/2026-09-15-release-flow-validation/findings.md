...
- Production-side signed probe result: a deliberately invalid manifest signed with the API container's own `RELEASE_HOOK_SECRET` passed signature validation and failed later as `release_hook_manifest_invalid`. Therefore the persistent GitHub Hook 403 on attempt 4 is not caused by production's secret or HMAC implementation; GitHub Actions is still signing with a different `GAP_RELEASE_HOOK_SECRET`.

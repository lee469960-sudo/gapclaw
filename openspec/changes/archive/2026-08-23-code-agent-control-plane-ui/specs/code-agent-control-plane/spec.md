...
系统 SHALL 允许项目管理者为 Code Project 创建和编辑草稿 Manifest，并在完整性校验通过后发布不可变版本。Manifest SHALL 至少表达受管仓库、基线 commit、允许路径、验证计划、可信镜像、允许工具和资源预算；历史已发布版本 SHALL 可查看但不可被修改。

#### Scenario: 保存未完成草稿
- **WHEN** 管理者保存尚未具备全部必填约束的 Manifest
- **THEN** 系统保存为草稿并标出缺失或无效字段
- **AND** 该草稿不得用于创建 Code run

#### Scenario: 发布有效 Manifest
- **WHEN** 管理者提交包含全部必填约束且通过策略校验的草稿
- **THEN** 系统发布一个新的版本化 Manifest
- **AND** 后续 Code run 使用该已发布版本冻结任务契约

#### Scenario: 发布无效 Manifest 被拒绝
- **WHEN** 管理者尝试发布缺少必填约束、违反允许范围或请求不支持环境的 Manifest
- **THEN** 系统拒绝发布并返回字段级可行动原因
- **AND** 不替换当前已发布版本

### Requirement: 控制面展示项目可用性和运行前门禁
系统 SHALL 向有权用户展示每个项目是否具备可用于新 Code run 的已发布 Manifest，以及不可用时的稳定原因。该状态 SHALL 区分项目停用、未发布 Manifest、Manifest 无效和环境 tier 不支持。

#### Scenario: 项目已就绪
- **WHEN** 项目已启用、环境 tier 受支持且具有有效已发布 Manifest
- **THEN** 控制面将其标记为可用于新 Code run

#### Scenario: 项目未就绪
- **WHEN** 项目因停用、未发布或无效 Manifest 或不支持的环境 tier 而不可用
- **THEN** 控制面显示对应的不可用原因和下一步操作指引

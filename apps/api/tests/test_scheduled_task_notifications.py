...
def test_notification_outbox_delivers_or_retries_without_agent_execution():
    ...
    try:
        db.add_all((
            ...
            ScheduledTaskRun(id="run", task_id="task", occurrence_key="one", state="succeeded", scheduled_for=now, available_at=now, chat_message_id=1),
            ScheduledTaskRun(id="run2", task_id="task", occurrence_key="two", state="succeeded", scheduled_for=now, available_at=now, chat_message_id=1),
        ...
        terminal = serialize_scheduled_run_result(db, db.get(ScheduledTaskRun, "run2"))
        assert terminal["state"] == "succeeded"
        assert terminal["notification_state"] == "failed"
        assert terminal["notification_warning"]
